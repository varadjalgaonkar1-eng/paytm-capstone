...
for i in range(500):
    ...
    status = np.random.choice(
        ["captured", "failed", "chargeback"], p=[0.92, 0.06, 0.02]
    )
    rows.append({
...
# 4. Injected 15 "burner account" chargeback frauds
next_user_id = 351
for i in range(15):
    txn_time = window_start + timedelta(
        days=random.randint(10, 29), hours=random.randint(0, 23)
    )
    signup = txn_time - timedelta(days=random.randint(1, 25))
    users = pd.concat(
        [
            users,
            pd.DataFrame([{"user_id": next_user_id, "signup_date": signup}]),
        ],
        ignore_index=True,
    )
    rows.append({
...
# 5. Injected 8 velocity-attack clusters (4 rapid txns within 5 minutes)
...
# 6. Build the discrepant gateway export
gateway = ledger.copy()
n = len(gateway)
...
mismatch_idx = np.random.choice(
    len(gateway), size=int(0.03 * n), replace=False
)
gateway.loc[mismatch_idx, "amount_inr"] = gateway.loc[
    mismatch_idx, "amount_inr"
